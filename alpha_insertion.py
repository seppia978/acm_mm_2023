...
def main(args):
    ...
    for idx, (img, lab) in tqdm.tqdm(enumerate(val_loader)):
        ...
        with torch.inference_mode():
            ...
            uscores = torch.softmax(model(img, labels=lab),1)
        ...
        torch.save(scores_array, f'alpha_res/plots/insertion_{baseline}_{arch_name}_{dataset}_normal.pt')
        torch.save(others_array, f'alpha_res/plots/insertion_{baseline}_{arch_name}_{dataset}_others.pt')
        torch.save(scores_std, f'alpha_res/plots/insertion_{baseline}_{arch_name}_{dataset}_nstd.pt')
        torch.save(others_std, f'alpha_res/plots/insertion_{baseline}_{arch_name}_{dataset}_otstd.pt')
        torch.save(counter, f'alpha_res/plots/insertion_{baseline}_{arch_name}_{dataset}_counter.pt')
        torch.save(torch.tensor([TOT]), f'alpha_res/plots/insertion_{baseline}_{arch_name}_{dataset}_tot.pt')
