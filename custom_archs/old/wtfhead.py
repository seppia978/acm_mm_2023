...
class WTFHead(WTFLayer):
    def __init__(self, attn: nn.Module, m:float = .5, classes_number=1000):
        ...
        self.alpha = nn.Parameter(torch.ones(self.classes_number, self.attn.proj.out_features, dtype=torch.float) *amax)
    ...
    def _attn_forward(self, x, w, b):
        ...
        # attn = torch.sigmoid(self.alpha)[self.label][(None,)+(...,)+(None,)*2]\
        #      * attn
    ...
    def forward(self, x):
        ...
        weight = self.attn.proj.weight * \
                 torch.sigmoid(self.alpha[self.label].squeeze())[(...,) + (None,) * dims]
        # weight = torch.sigmoid(self.alpha[self.label].squeeze())

        if self.attn.proj.bias is not None:
            # bias = self.conv.bias * alpha[self.label].squeeze()
            bias = self.attn.proj.bias * torch.sigmoid(self.alpha[self.label].squeeze())
    ...
    if isinstance(model, VisionTransformer):
        for name, x in model.named_modules():
            if isinstance(x, Block):
                if not sigmoid:
                    ret[name] = x.attn.alpha.data
                else:
                    ret[name] = torch.sigmoid(x.attn.alpha.data)
    return ret
