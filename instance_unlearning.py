...
def parameters_distance(model:Union[torch.Tensor, Iterable], other:Union[torch.Tensor, Iterable], kind:str = 'l2'):
        ...
        for i, (x,x1) in enumerate(zip(model.model.parameters(), other.parameters())):
            x, x1 = x.cuda(), x1.cuda()
            if kind.lower() == 'l1':
                    ret.append(
...
def main(args):
    ...
    clamp = args.clamp
    flipped = False
    n_imgs_4_instance_unl = args.num_imgs_4_instance_unlearning


    assert n_imgs_4_instance_unl > 0, f'n_imgs_4_instance_unl must not be below zero. found {n_imgs_4_instance_unl}'
    ...
    hyperparams = {
        ...
        'clamp': clamp,
        'flipped': flipped,
        'n_imgs_4_instance_unl': n_imgs_4_instance_unl
    }
    ...
    if dataset.lower() == 'imagenet':
        ...
        rnd_perm = torch.randperm(len(_train))
        unl_idx = rnd_perm[:n_imgs_4_instance_unl]
        unlearning_set = Subset(_train, unl_idx)
        unlearning_set.targets = torch.tensor(_train.targets)[unl_idx].tolist()
        ret_idx = rnd_perm[n_imgs_4_instance_unl:]
        retaining_set = Subset(_train, ret_idx)
        retaining_set.targets = torch.tensor(_train.targets)[ret_idx].tolist()
        train = ConcatDataset((unlearning_set, retaining_set))
        train.targets = [*unlearning_set.targets, *retaining_set.targets]

        # if 'zero' not in hyperparams['loss_type']:
        #     train.samples_weight = torch.tensor(
        #         [
        #             *[.5/train.cumulative_sizes[0] for _ in range(train.cumulative_sizes[0])],
        #             *[.5/(train.cumulative_sizes[1] - train.cumulative_sizes[0]) \
        #                 for _ in range(train.cumulative_sizes[1] - train.cumulative_sizes[0])]
        #         ]
        #     )
        #     train.sampler = WeightedRandomSampler(train.samples_weight.type('torch.DoubleTensor'), len(train.samples_weight))
    ...
    def train_loop(
        ...
        validation_frequency = 4 # int(len(train)/1000) if int(len(train)/1000) > 0 else 10
        ...
        for epoch in range(n_epochs):
            ...
            if 'zero' in hyp['loss_type']:
                batch_train = len(train.datasets[0])

                total_len = train.cumulative_sizes[0]
            else:
                total_len = train.cumulative_sizes[-1]

            unlearning_loader = DataLoader(train.datasets[0], batch_size=batch_train//2, num_workers=1, shuffle=True)
            unlearning_loader_iter = iter(unlearning_loader)
        
            retaining_loader = DataLoader(train.datasets[1], batch_size=batch_train//2, num_workers=4, shuffle=True)
            retaining_loader_iter = iter(retaining_loader)

            # loss_ctrain_list, loss_ottrain_list, loss_otval_list, loss_cval_list = [], [], [], []

            for idx in range(total_len // batch_train):
                model.train()
                print(f'Untraining: {round((100 * batch_train * idx)/train.cumulative_sizes[-1],2)}%')

                try:
                    unlearnt_imgs, unlearnt_labels = next(unlearning_loader_iter)
                except:
                    if 'zero' in hyp['loss_type']:
                        batch_train = len(train.datasets[0])
                        unlearning_loader = DataLoader(train.datasets[0], batch_size=batch_train, num_workers=1, shuffle=True)
                        unlearning_loader_iter = iter(unlearning_loader)
                    else:
                        unlearning_loader = DataLoader(train.datasets[0], batch_size=batch_train//2, num_workers=1, shuffle=True)
                        unlearning_loader_iter = iter(unlearning_loader)
                ...
                unlearnt_imgs = unlearnt_imgs.to(device)
                unlearnt_labels = unlearnt_labels.to(device)
                ...
                if not 'zero' in hyp['loss_type']:
                    ...
                    # )
                    kept_imgs, kept_labels = next(retaining_loader_iter)
                    kept_imgs = kept_imgs.cuda()
                    kept_labels = kept_labels.cuda()
                ...
                if idx % validation_frequency == 0 and validation:
                    ...
                    max_val_keep_size = min(hyp['max_val_size'], len(val))
                    ...
                    val_loader = DataLoader(
                        random_split(val, [max_val_keep_size, len(val)-max_val_keep_size])[0],
                        batch_size=batch_val, num_workers=4, shuffle=True
                    )
                    unlearning_training_test = DataLoader(
                        random_split(train.datasets[0], [len(train.datasets[0]), 0])[0],
                        batch_size=batch_val, num_workers=4, shuffle=True
                    )
                    # unlearning_training_test = DataLoader(
                    ...
                    with torch.inference_mode():
                        ...
                        mean_acc_keep /= len(val_loader.dataset)
                        ims.cpu()

                        mean_acc_unl_train = 0.
                        for ival, (ims, labs) in enumerate(unlearning_training_test):
                            ims = ims.cuda()

                            outs = torch.softmax(model(ims), -1).cpu()

                            mean_acc_unl_train += (outs.max(1).indices == labs).sum()

                        mean_acc_unl_train /= len(unlearning_training_test.dataset)
                        ims.cpu()

                        mean_acc_ret_train = 0.
                        for ival, (ims, labs) in enumerate(retaining_training_test):
                            ims = ims.cuda()

                            outs = torch.softmax(model(ims), -1).cpu()

                            mean_acc_ret_train += (outs.max(1).indices == labs).sum()

                        mean_acc_ret_train /= len(retaining_training_test.dataset)
                        ims.cpu()
                    if not debug:
                        run.log({'acc_of_kept': mean_acc_keep})
                        run.log({'acc_unl_train': mean_acc_unl_train})
                        run.log({'acc_ret_train': mean_acc_ret_train})
                    ...
                    current_acc = 0.5 * ((1-mean_acc_unl_train) + mean_acc_ret_train)
                    if evaluation_frequency and idx % evaluation_frequency == 0:
                        if current_acc < best_acc:
                            patience -= 1
                            currents.append(current_acc)
                            if patience == 0 or mean_acc_ret_train < 0.4:
                                should_stop = True
                        else: 
                            best_acc = current_acc
                            best_acc_both = (mean_acc_ret_train, mean_acc_unl_train)
                            currents = list()
                            patience = hyp['initial_patience']
                            best_found = True
                    if not debug:
                    ...
                    if should_stop:
                        print(f'mean_unl: {mean_acc_unl_train}, current: {currents}, best: {best_acc}, patience: {patience}')
                ...
                # if idx % save_checkpoint_frequency == 0 or best_found:
                    ...
                    PATH = f"{run_root}/last_intermediate.pt" if not best_found else f"{run_root}/best_CLASS.pt"
    ...
    mean_ret_acc = ret_acc
    mean_unl_acc = unl_acc
    ...
    print(f'ret: {ret_acc}, unl: {unl_acc}')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-k", "--num-imgs-4-instance-unlearning", type=int, help='Num imgs 4 instance unlearning', default=0)
