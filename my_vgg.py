...
class AlphaWeightedConv2d(nn.Module):
    def __init__(self, conv: nn.modules.Conv2d):
        ...
        self.conv = conv
        self.alpha = Parameter(torch.randn(self.conv.in_channels))


    def forward(self, x):

        self.alpha = nn.functional.sigmoid(self.alpha)
        weights = self.conv.weight * self.alpha
        biases = self.conv.bias * self.alpha
        x = self.conv1._conv_forward(x, weights, biases)
...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False) -> nn.Sequential:
    ...
    for v in cfg:
        if v == "M":
            ...
            awconv2d = AlphaWeightedConv2d(conv2d)
            if batch_norm:
                layers += [awconv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
            else:
                layers += [awconv2d, nn.ReLU(inplace=True)]
            in_channels = v
