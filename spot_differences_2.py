...
general = convert_conv2d_to_alpha(models.vgg16(pretrained=True))
...
ws = pyxlutils.create_ws_2(ws=ws, cname='shark')
...
def f(a, b, start_row=1,method='diff', N=True):
    ...
    for i, (aa, bb) in enumerate(zip(a.named_parameters(), b.named_parameters())):
        ...
        if '28' in aa[0]:
            ...
            # try:
            #     plot_pairs(xa, wei_xb, idx=0, layer=aa[0], idx2=0, inc=len(xa),alpha=alpha,th=0.4)
        ...
        if 'alpha' in aa[0]:
            ws[f'{get_column_letter(col)}{row}'] = aa[0]

            ws, col = pyxlutils.write_alpha_layer(
                ws=ws,
                val=val,
                st_col=col
            )
...
diff,col=f(general,unlearnt,method='diff',N=False,start_row=3)

ws = pyxlutils.color_scale(ws=ws, cols=[get_column_letter(c) for c in range(1,col)])
