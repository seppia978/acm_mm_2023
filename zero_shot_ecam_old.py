...
def main(args):
    ...
    if 'vgg16'in arch_name:
        ...
        model.arch.blocks[-1].norm1.register_forward_hook(hook_fn)
    ...
    def train_loop(
        ...
        # cam = EigenCAM(model=model.arch, target_layers=[tlayer])
        # c_to_del = [0]
            ...
            for idx, (imgs, labels) in enumerate(train_loader):
                ...
                if hyp['loss_type'] == 'difference':
                    ...
                    if 'multiplication' in hyp['loss_type']:
                        ...
                        global hooked_tensor
                        _, _, VT = torch.svd(hooked_tensor)
                        s = hooked_tensor @ VT[0]                
                        s.sub_(s.flatten(start_dim=-2).min(-1).values[(...,) + (None,) * 2])
                        s.div_(s.flatten(start_dim=-2).max(-1).values[(...,) + (None,) * 2] + 1e-15)
                        loss_cls = hyp['lambda1'] / s.mean()
                        loss_reg = torch.pow(hyp['lambda0'] * model.parameters_distance(torch.zeros(1).long(), kind='kl-div'), 2).max(0).values
                        alpha_norm = hyp['lambda2'] * model.minimum_by_layer().max().to('cuda')
